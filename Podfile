...
target 'BenchmarkDemo-libWebp' do
  ...
  pod 'BenchmarkCase_BuiltInOperators', :path => './BenchmarkCase_BuiltInOperators'
end

# ENABLE_OPT_IN_DEBUG：设置 libwebp、BenchmarkCase_BuiltInOperators 在 DEBUG 模式是否下开启编译优化
...
ENABLE_OPT_IN_DEBUG = true # true or false

post_install do |installer|

  installer.pods_project.targets.each do |target|

    if target.name == 'libwebp' || target.name == 'BenchmarkCase_BuiltInOperators'
